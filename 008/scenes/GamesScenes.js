class GamesScenes extends Phaser.Scene {
    constructor() {
        ...
        this.road = null;
        this.player = null;
        this.playerShadow = null; // 🌟 ตัวแปรเก็บเงาตัวละคร
        this.cursors = null;
        this.wasd = null;
    ...
    preload() {
        ...
        this.load.audio('crash_sfx', 'sound/Sound_of_a_collision.mp3'); 
    ...
    create() {
        ...
        // ==========================================
        // 🌟 อัปเกรดกราฟิก: สร้าง Texture ถนนและแสงเงา
        // ==========================================
        let graphics = this.make.graphics({ x: 0, y: 0, add: false });
        
        // 1. พื้นหญ้า (ฐาน)
        graphics.fillStyle(0x2c6b35); 
        graphics.fillRect(0, 0, 400, 200);
        // สร้าง Texture หญ้าด้วยจุดสีเขียวสุ่ม
        for (let i = 0; i < 400; i++) {
            graphics.fillStyle(Phaser.Math.RND.pick([0x3e8948, 0x1f4d25, 0x4caf50]), 0.6);
            graphics.fillRect(Phaser.Math.Between(0, 400), Phaser.Math.Between(0, 200), 4, 4);
        }

        // 2. พื้นถนนยางมะตอย (ฐาน)
        graphics.fillStyle(0x202b38); 
        graphics.fillRect(40, 0, 320, 200);
        // สร้าง Texture ยางมะตอยด้วยจุดสีดำ/เทาสุ่ม
        for (let i = 0; i < 600; i++) {
            graphics.fillStyle(Phaser.Math.RND.pick([0x1a232d, 0x2c3e50, 0x111111]), 0.5);
            graphics.fillRect(Phaser.Math.Between(40, 360), Phaser.Math.Between(0, 200), 3, 3);
        }

        // 3. เส้นขอบถนนและเส้นกลาง
        ...
        // 4. แสงเงา (Shadow Gradients) สร้างมิติลึกตื้น
        // เงาต้นไม้ริมขอบจอ
        graphics.fillGradientStyle(0x000000, 0x000000, 0x000000, 0x000000, 0.9, 0, 0.9, 0);
        graphics.fillRect(0, 0, 40, 200);
        graphics.fillGradientStyle(0x000000, 0x000000, 0x000000, 0x000000, 0, 0.9, 0, 0.9);
        graphics.fillRect(360, 0, 40, 200);
        
        // เงาขอบถนน
        graphics.fillGradientStyle(0x000000, 0x000000, 0x000000, 0x000000, 0.5, 0, 0.5, 0);
        graphics.fillRect(45, 0, 20, 200);
        graphics.fillGradientStyle(0x000000, 0x000000, 0x000000, 0x000000, 0, 0.5, 0, 0.5);
        graphics.fillRect(335, 0, 20, 200);
        ...
        // 🌟 เพิ่มเงาใต้เท้าตัวละคร
        this.playerShadow = this.add.ellipse(200, 500, 35, 15, 0x000000, 0.4).setDepth(19);
    ...
    update() {
        ...
        // 🌟 อัปเดตตำแหน่งเงาให้ตามเท้าตัวละครตลอดเวลา
        if (this.playerShadow) {
            this.playerShadow.x = this.player.x;
            this.playerShadow.y = this.player.y + 40; 
            // ทำให้เงากะพริบจางลงพร้อมตัวละครตอนโดนชน
            this.playerShadow.alpha = this.player.alpha * 0.4;
        }
    ...
    gameOver() {
        ...
        this.player.alpha = 1;
        this.player.anims.stop();
        if (this.playerShadow) this.playerShadow.destroy(); // ลบเงาทิ้งตอนตาย
