export default class BossZombie extends Phaser.Physics.Arcade.Sprite {
    ...
    createBossHUD() {
        ...
        this.bossName = this.scene.add.text(400, 42, 'ซอมกลายพันธุ์', {
            fontSize: '16px', fill: '#ff0000', fontStyle: 'bold', stroke: '#000000', strokeThickness: 3
        }).setOrigin(0.5);
    ...
    die() {
        ...
        try { this.play('boss1_death_anim', true); } catch (e) { console.warn("ข้ามแอนิเมชันที่รูปหายไป"); }
    ...
    updateAI(player) {
        ...
        if (!this.isAttacking) {
            if (distance > 150) { 
                ...
                try { this.play('boss1_walk_anim', true); } catch (e) {} 
            } else {
                if (this.canAttack) {
                    this.setVelocity(0, 0);
                    ...
                    this.setVelocity(0, 0);
                    this.stop();
                    this.setTexture('boss1_walk_000'); 
                }
            }
        }
    }
    ...
    performAttack1(player) {
        ...
        try { this.play('boss1_attack1_anim', true); } catch (e) {}
