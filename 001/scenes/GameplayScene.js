...
export default class GameplayScene extends Phaser.Scene {
    ...
    init(data) {
        ...
        // รีเซ็ตสถานะความคืบหน้าของเควสต์และบทสนทนาทั้งหมดสำหรับรอบเล่นใหม่
        this.hasReachedExitArea = false;
        this.hasKeycard = false;
        this.hasSupplyBox = false;
        this.hasDeliveredSupplies = false;
        this.hasObtainedFormula = false;
        this.hasCraftedChemical = false;
        this.hasMeltedOfficeDoor = false;
        this.hasCompletedQuest5 = false;
        this.hasPlayedOfficeEntryDialogue = false;
        this.hasPlayedKeycardDoorDialogue = false;
        this.hasPlayedSmallOfficeDeniedDialogue = false;
        this.hasPlayedOfficeDeniedDialogue = false;
    ...
    create() {
        ...
        // หากเป็นการกด Continue ฟื้นคืนชีพ: โหลดสถานะเควสต์และไอเทมเดิมก่อนตายกลับมา
        if (this.isContinueRevive) {
            const savedState = this.game.registry.get('savedQuestState');
            if (savedState) {
                if (this.quest) {
                    if (savedState.currentQuest) this.quest.currentQuest = { ...savedState.currentQuest };
                    if (savedState.completedQuests) this.quest.completedQuests = savedState.completedQuests.map(q => ({ ...q }));
                }
                if (savedState.inventory) {
                    Object.assign(this, savedState.inventory);
                }
            }
            if (this.quest && this.quest.currentQuest) {
                this.quest.showNotification(this.quest.currentQuest, false);
            }
        } else if (this.currentMapKey === 'break_room') {
            ...
            // เมื่อเริ่มหรือรีสตาร์ทห้อง Lab ใหม่: เควสต์ที่ 1 สำเร็จแล้ว และเริ่มต้นเควสต์ที่ 2
            if (this.quest) {
    ...
    triggerPlayerDeath() {
        ...
        // 4. บันทึกสถานะเควสต์และไอเทมปัจจุบันสำหรับกรณีที่ผู้เล่นเลือก CONTINUE
        if (this.quest) {
            this.game.registry.set('savedQuestState', {
                currentQuest: this.quest.currentQuest ? { ...this.quest.currentQuest } : null,
                completedQuests: this.quest.completedQuests ? this.quest.completedQuests.map(q => ({ ...q })) : [],
                inventory: {
                    hasKeycard: this.hasKeycard,
                    hasSupplyBox: this.hasSupplyBox,
                    hasDeliveredSupplies: this.hasDeliveredSupplies,
                    hasObtainedFormula: this.hasObtainedFormula,
                    hasCraftedChemical: this.hasCraftedChemical,
                    hasMeltedOfficeDoor: this.hasMeltedOfficeDoor,
                    hasCompletedQuest5: this.hasCompletedQuest5,
                    hasPlayedOfficeEntryDialogue: this.hasPlayedOfficeEntryDialogue,
                    hasPlayedKeycardDoorDialogue: this.hasPlayedKeycardDoorDialogue,
                    hasPlayedSmallOfficeDeniedDialogue: this.hasPlayedSmallOfficeDeniedDialogue,
                    hasPlayedOfficeDeniedDialogue: this.hasPlayedOfficeDeniedDialogue,
                    hasReachedExitArea: this.hasReachedExitArea
                }
            });
        }

        // 5. เพิ่มจำนวนรอบการตาย
        const currentRestarts = (this.game.registry.get('restartCount') || 0) + 1;
        this.game.registry.set('restartCount', currentRestarts);

        // 6. ให้เวลาแสดงอนิเมชั่นล้มลงจนเสร็จสมบูรณ์ (1.6 วินาที) ก่อนเฟดจอดำเข้าสู่ GameOverScene
